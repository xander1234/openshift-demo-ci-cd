...
GITHUB_REF=${GITHUB_REF:-ocp-3.11-bg}
...
function deploy() {
  ...
  local template=https://raw.githubusercontent.com/$GITHUB_ACCOUNT/openshift-demo-ci-cd/$GITHUB_REF/cicd-template.yaml
  ...
  oc $ARG_OC_OPS new-app -f $template -p DEV_PROJECT=dev-$PRJ_SUFFIX -p STAGE_PROJECT=stage-$PRJ_SUFFIX -p PROD_PROJECT=prod-$PRJ_SUFFIX -p DEPLOY_CHE=$ARG_DEPLOY_CHE -p EPHEMERAL=$ARG_EPHEMERAL -p ENABLE_QUAY=$ARG_ENABLE_QUAY -p QUAY_USERNAME=$ARG_QUAY_USER -p QUAY_PASSWORD=$ARG_QUAY_PASS -n cicd-$PRJ_SUFFIX 
}
